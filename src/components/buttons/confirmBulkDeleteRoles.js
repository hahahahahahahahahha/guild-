...
export async function execute(interaction) {
  ...
  for (let i = 0; i < roleIds.length; i++) {
    ...
    try {
      ...
      deletedRoles.push({ id: roleId, name: roleName });
  ...
  const embed = new EmbedBuilder()
    ...
    .setColor(deletedRoles.length > 0 ? '#00FF00' : '#FF0000')
    .setDescription(
      `**Successfully deleted ${deletedRoles.length}/${roleIds.length} roles**\n\n` +
      (deletedRoles.length > 0 ? `**Deleted Roles:**\n${deletedRoles.map(role => `• ${role.name}`).join('\n')}\n\n` : '') +
