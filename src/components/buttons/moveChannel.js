...
export const customId = 'move_channel';
...
export async function execute(interaction) {
  ...
  try {
    ...
    if (nonCategoryChannels.size === 0) {
      // No channels available to move
      const errorEmbed = createErrorEmbed(
        'No Channels Available',
        'There are no non-category channels available to move.'
      );
    ...
    const embed = createPanelEmbed(
      'Move Channel',
      '> Select a channel to move from the menu below.\n\n' +
      'After selecting a channel, you will be able to move it up or down in the channel list.'
    );
    ...
    const selectMenu = new StringSelectMenuBuilder()
      ...
      .setPlaceholder('Select a channel to move')
