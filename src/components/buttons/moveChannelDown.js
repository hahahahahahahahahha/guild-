...
export const customId = 'move_channel_down';
...
export async function execute(interaction) {
  ...
  try {
    ...
    const loadingEmbed = createLoadingEmbed(
      'Moving Channel',
      `Moving channel **${channel.name}** down in the list...`
    );
    ...
    // Move the channel down (higher position number)
    await channel.setPosition(currentPosition + 1, { reason: `Moved by ${interaction.user.tag}` });
