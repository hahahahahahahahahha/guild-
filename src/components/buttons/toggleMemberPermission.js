...
export const customId = 'toggle_member_permission';
...
export async function execute(interaction) {
  ...
  const [channelId, memberId, permission, categoryIndex] = interaction.customId.split(':').slice(1);
  ...
  try {
    ...
    if (newState === 'allow') {
      ...
      const currentOverwrites = channel.permissionOverwrites.cache.get(memberId);
      if (currentOverwrites) {
        ...
        if (allowPermissions.bitfield === 0n && denyPermissions.bitfield === 0n) {
          await channel.permissionOverwrites.delete(memberId, `Permission reset by ${interaction.user.tag}`);
        } else {
    ...
    await interaction.reply({
      embeds: [createSuccessEmbed(
        'Permission Updated',
        `Successfully ${newState === 'allow' ? 'allowed' : newState === 'deny' ? 'denied' : 'reset'} the **${getPermissionName(permission)}** permission for **${member.user.tag}** in **${channel.name}**.`
      )],
      ephemeral: true
    });
  } catch (error) {
  ...
  const updatedPermissions = channel.permissionOverwrites.cache.get(memberId);
  ...
  for (const perm of selectedCategory.permissions) {
    ...
    const button = new ButtonBuilder()
      .setCustomId(`toggle_member_permission:${channelId}:${memberId}:${perm}:${categoryIndex}`)
