...
export async function execute(interaction) {
  ...
  try {
    ...
    const loadingEmbed = createLoadingEmbed(
      'Moving Role',
      `Moving role **${role.name}** down in the hierarchy...`
    );
    ...
    // Move the role down (lower position number)
    // Make sure we don't go below position 1 (position 0 is @everyone)
    const newPosition = Math.max(1, currentPosition - 1);
    await role.setPosition(newPosition, { reason: `Moved by ${interaction.user.tag}` });
