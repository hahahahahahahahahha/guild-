...
export const customId = 'channel_perm_category_select';
...
export async function execute(interaction) {
  ...
  const embed = createPanelEmbed(
    `${selectedCategory.name}: ${role.name} in ${channel.name}`,
    `> Manage **${selectedCategory.name.toLowerCase()}** for the **${role.name}** role in the **${channel.name}** channel.\n\n` +
  ...
  for (const permission of selectedCategory.permissions) {
    ...
    const button = new ButtonBuilder()
      .setCustomId(`toggle_channel_permission:${channelId}:${roleId}:${permission}:${categoryIndex}`)
